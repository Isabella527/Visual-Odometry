...
def VO_localization_mapping(i_frame, img, img_rgb, img_pre, S_pre, C_pre, K, param, display_process=False):
    ...
    if len(C_pre.keypoints) > 0:
        ...
        for i in C_index_match:
            ...
            # Additional error checking: filter out negative depths and points too far away
            is_valid = True
            if index_pW_inliers.shape[0] > 0:
                # Check for negative depth (Z coordinate should be positive)
                if p_W_i[2, 0] < 0:
                    is_valid = False
                # Check for points too far away (beyond reasonable distance, e.g., 100 meters)
                elif np.linalg.norm(p_W_i[:, 0]) > 100:
                    is_valid = False

            # If the triangulated p_W is valid inlier then the corresponding keypoint is added to the state
            if index_pW_inliers.shape[0] > 0 and is_valid:
